...
select customer_id
    from customer c
    where zip_code = '22030';

// query 2
select transaction_id
from TRANSACTION_CONTAINS tc join Product p
    on tc.upc = p.upc 
    where p.category  = 'Deli';

//query 3
select distinct t.customer_id
from transactions t join TRANSACTION_CONTAINS tc  
    on tc.transaction_id = t.transaction_id 
    where tc.upc in (select UPC 
        from PRODUCT
        where brand in ('Pepsi','Coca-Cola'));

//query 4
select distinct t.customer_id
from transactions t
join TRANSACTION_CONTAINS tc  
on tc.transaction_id = t.transaction_id 
where tc.upc in (select UPC 
    from PRODUCT
    where brand in ('Pepsi'))
INTERSECT
select distinct t.customer_id
from transactions t 
join TRANSACTION_CONTAINS tc  
on tc.transaction_id = t.transaction_id where
tc.upc in (select UPC 
    from PRODUCT
    where brand in ('Coca-Cola'));
...
SELECT c.customer_ID, SUM(tc.quantity) AS total_quantity
FROM Transactions t
JOIN Transaction_Contains tc ON t.transaction_ID = tc.transaction_ID
JOIN Product p ON tc.UPC = p.UPC
...
((select UPC
    from product p
    where p.category = 'Dairy') MINUS (select UPC
        from Transactions t, Transaction_Contains tc
        where t.transaction_id = tc.transaction_id and
